...
- **Nicolas Aunai's affiliation is the ROR-backed `Laboratory of Plasma Physics (LPP/CNRS)` / `https://ror.org/05c95bg36`** — the same organization recorded for Jeandet. Both independent sources for Aunai name that one laboratory (LPP, UMR 7648): his ORCID employment record reads "Laboratoire de Physique des Plasmas / CNRS", and the shared byline of the 2025 abstracts reads "CNRS, Laboratory Of Plasma Physics, Palaiseau CEDEX, France". The ROR-identified name is the correct expression of both.
