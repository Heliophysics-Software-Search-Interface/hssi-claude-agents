...
### 33. Logo (OPTIONAL)
https://media.githubusercontent.com/media/JHUAPL/kaiju/3382b1f2fd39419e5a43d4c7bc5427029d48291a/docs/source/_static/MAGE_Logo_final_dark-bg_vertical.png

*Source: docs/source/conf.py lines 37-41 designates docs/source/_static/MAGE_Logo_final_dark-bg_vertical.png (the MAGE logo, 1168x1444 PNG) as the documentation logo. The repository stores the file as a Git LFS pointer (since commit a3db44a6, "Store PNG images in Git LFS", 2025-12-03), so the media.githubusercontent.com endpoint is used because it resolves the LFS object to the actual image (128,262 bytes, sha256 c310f5cecfa73d4fd73d51a2d8a787863d9641132721ddf59d669baaa1bae25c, byte-identical to the pre-LFS blob added in commit ed2428f5). The URL is pinned to master commit 3382b1f2 (2026-08-24) rather than the master ref so that later moves or renames of the file on master do not affect it, and its responses permit cross-origin image embedding.*

*Superseded value: https://raw.githubusercontent.com/JHUAPL/kaiju/master/docs/source/_static/MAGE_Logo_final_dark-bg_vertical.png — after the 2025-12-03 LFS conversion, raw.githubusercontent.com serves that path as the 131-byte LFS pointer text (text/plain), which browsers render as a broken image; it was already broken at the 2026-07-24 extraction. Re-evaluate this field if the pinned commit becomes unreachable (history rewrite or pruning), if the project stops tracking the logo in LFS (a plain raw URL would then suffice), or if conf.py designates a different logo. The ReadTheDocs copy at https://kaiju-docs.readthedocs.io/en/latest/_static/MAGE_Logo_final_dark-bg_vertical.png serves a recompressed rendition of the same image and is the fallback if the media endpoint becomes unavailable.*
