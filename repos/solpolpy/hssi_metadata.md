...
**Seeds used:** live HSSI record (`GET http://localhost/api/view/software/da5cb52c-1e0a-4c22-ab96-e05c210c0e60/`, authoritative baseline) and the prior partial `hssi_metadata.md` (Field 4 only, explicitly marked incomplete — its values were re-evaluated against the current source, not assumed correct).

**Applied to HSSI:** 2026-07-28. Fields 2–33 match live HSSI.
...
*Change vs. the prior partial file: kept its `Coordinate Transforms`, `Coordinate Transforms: Mission-Specific`, `Data Processing and Analysis`, `Data Processing and Analysis: Analysis`, `Data Processing and Analysis: Image Processing`, `Data Processing and Analysis: Processing`, `Data Visualization`, `Data Visualization: 2D Graphics`, `Mission-related`, `Mission-related: Science Data Processing`. Added `Coordinate Transforms: Solar`, `Data Processing and Analysis: Calibration`, `Data Processing and Analysis: Wave Polarization Analysis`, `Data Visualization: Mission-Specific`, `Mission-related: Calibration`.*
...
*Source note: Names, ORCIDs and affiliations are from the existing HSSI record; affiliation organization names and RORs were resolved from HSSI's `Organization` table (UUIDs `7d4f0f58…` = Southwest Research Institute, `dd809fc7…` = European Space Research and Technology Centre, `ade5c922…` = NCAR High Altitude Observatory). `CITATION.cff` lists exactly the same nine people with the same eight ORCIDs and likewise gives Walbridge no ORCID. DataCite (concept DOI 10.5281/zenodo.10076326) lists the same nine creators with the same ORCIDs, and also carries no identifier for Walbridge. A public ORCID registry search for `family-name:Walbridge AND given-names:Bryce` returned 0 results, so his identifier stays empty — nothing invented. `pyproject.toml` contributes five author entries with emails (`J. Marcus Hughes`, `Matthew J. West`, `Ritesh Patel`, `Bryce M. Walbridge`, `Chris Lowder`) — all five are already in the list; HSSI/`CITATION.cff` name forms are retained rather than adopting the `pyproject.toml` middle-initial variants (see Open Questions). No author added or removed.*
...
*Documented alternatives for the Version Description, kept visible for the approval gate rather than discarded — the user may choose either at the gate:*
1. ***The raw GitHub markdown body***, i.e. the release body exactly as the GitHub API returns it, with `## What's Changed`, seven `* ` bullets and `**Full Changelog**:` intact. Highest fidelity to the upstream release notes, but it would be the only such value in the corpus (see the 127/0/0/1 counts above).
2. ***A hand-written summary derived from `CHANGELOG.rst`*** (the extractor's original proposal): "New features: `mzpsolar_to_npol` now accepts `POLAROFF` (#209); adds a new function to generate the alpha matrix using the WCS (#217). Bug fix: fixes the solar-instrument roundtrip issue for the MZP transform (#218)." Shortest and most readable, but it breaks the convention that this field carries the release body rather than an editorial summary.
...
*Source note: PROPOSED REPLACEMENT of the live HSSI value `Creative Commons Attribution 4.0 International`. Full evidence and reasoning are in the Change Log below. Value and URI copied verbatim from the live HSSI `License` vocabulary row.*
...
*Source note: Entry 1 is the existing HSSI value, now resolved to its SPASE identifier — confirmed exactly by looking up HSSI's own `relatedObservatories` row UUID `d882f614-6240-4f53-a423-295236b5c27b` in the `InstrumentObservatory` table, which returns precisely this `name` + `identifier` pair (no rename, no duplicate risk). Entries 2-5 are ADDITIONS. Every value carries its SPASE identifier. Full per-candidate resolution log in the Change Log below.*
...
The extractor's normal source priority puts Zenodo/DataCite above manual examination, but that ordering is for filling gaps — it does not let an archive's deposit-record default override the software's own explicit legal license grant, and HSSI Field 15 asks for "the full name of the license **assigned to this software**". `GNU Lesser General Public License v3.0 only` is the exact SPDX title for `LGPL-3.0-only` and is present verbatim in the live HSSI `License` vocabulary. Because this replaces a submitted value, it is left visible here for validator/user approval rather than treated as settled.

**A2. Field 12 (Version): REPLACE the live version entry — stored `number` `0.5.1` -> `0.7.0`.**

*Render vs. store.* The prefixed string `solpolpy - 0.5.1` seen in the USER view is a **rendered display form, not a stored value**: `SoftwareVersion.__str__` in hssi-website is `f"{self.software.first()} - {self.number}"`, and the USER view serializes through `str(self)`. Verified directly against `http://localhost/api/models/SoftwareVersion/rows/all/`: of all **213** rows, **zero** contain `" - "` in `number`. The stored `number` therefore goes `0.5.1` -> **`0.7.0`**, **not** `solpolpy - 0.7.0`; storing the prefix would render as `solpolpy - solpolpy - 0.7.0`.

*Correction to an earlier claim in this file.* A previous revision of this entry stated that "live HSSI's version entry carried only the number string." **That was false.** The live row `b19751af-aa6c-4341-bc97-9143be8923c2` has all four sub-fields populated:
...
**The replacement overwrites all four sub-fields**, not just the number. Note the convention the live `description` reveals: it is the GitHub release body carried over, **markdown-stripped to plain text**, rather than a hand-written summary — so Field 12 follows that convention. Stripping is corpus-wide, not just this row's habit: of the 127 `SoftwareVersion` rows with a non-empty `description`, zero contain markdown bullets (`^* `) and zero contain bold (`**`), and exactly one contains a heading. The raw GitHub markdown body and a `CHANGELOG.rst`-derived hand summary are both recorded in Field 12 as documented alternatives for the approval gate.

*Version evidence.* Five independent authoritative sources agree 0.7.0, released 2026-07-28, is current: git tag `0.7.0` (newest of 19, dated 2026-07-28, at HEAD `a600958` "Merge pull request #221 from punch-mission/release-0.7.0"); `CITATION.cff` `version: 0.7.0` / `date-released: 2026-07-28`; `CHANGELOG.rst` "Version 0.7.0: July 28, 2026"; PyPI `info.version` 0.7.0 uploaded 2026-07-28T08:43:27; DataCite for the concept DOI `version: "0.7.0"`, `Issued 2026-07-28`. The live 0.5.1 entry is six releases behind, and issue #57's note that "0.6.1 is newer" is itself now out of date.

**A3. Exactly one removal is proposed, and it was explicitly approved by the user on 2026-07-28: Field 19 (Output File Formats), remove `FITS`.** Evidence: solpolpy writes no files — a repository-wide search for `writeto`, `.write(`, `to_fits` and file-open-for-write across `solpolpy/` and `tests/` returns zero matches, found independently by both the extractor and the validator. `resolve` returns in-memory `NDCollection`/`NDCube` objects, and `util.py::collection_to_maps` hands off `sunpy.map.Map` objects that the *caller* may serialize; Field 19 means formats the software itself writes, so no value is defensible. Field 19 therefore becomes "Not found". **Every other multi-valued live HSSI value is preserved:** `softwareFunctionality` (all 3 kept, 2 missing parents plus 10 new values added), `relatedRegion` (`Solar Environment` kept, `Interplanetary Space` added), `authors` (all 9 kept with their identifiers and affiliations), `inputFormats` (`FITS` kept — solpolpy definitively reads FITS), `cpuArchitecture`, `operatingSystem`, `programmingLanguage`, `relatedPhenomena` (`Solar Corona` kept), `relatedObservatories` (PUNCH kept, now identifier-resolved), `relatedSoftware` and `interoperableSoftware` (the punchbowl DOI kept in both).

**A4. Additions to previously empty or null live HSSI fields:** Field 14 Reference Publication, Field 16 Keywords, Field 17 Data Sources, Field 23 Development Status, Field 27 Related Publications, Field 31 Related Instruments; plus two Field 30 entries (ndcube, sunpy) and four Field 32 entries (SOHO, STEREO-A, STEREO-B, MLSO). Fields 25, 26, 28 and 33 remain empty ("Not found") — matching live HSSI, with reasons recorded in each field.

**A5. The remaining validation findings were reviewed and accepted as extracted (user decision, 2026-07-28).** W1 — the Field 15 License replacement to `GNU Lesser General Public License v3.0 only` is approved. W2 — `Data Processing and Analysis: Wave Polarization Analysis` is kept in Field 4. W3 — Sam Van Kooten is deliberately **not** added to Field 6: `CITATION.cff` curates authorship by intellectual contribution rather than commit history (four of the nine listed authors have zero commits), so his absence is a project choice, not a metadata gap. W5 — `Interplanetary Space` is kept in Field 5. S1 — Field 25 Funder stays "Not found". S3 — the `sunkit_image` exclusion from Fields 29 and 30 is accepted. S4 — the SECCHI roll-up to the two spacecraft-level rows, the MLSO SMWG namespace choice, and the PUNCH `.html` observatory row are all accepted as emitted. S5 — Bryce Walbridge's identifier and affiliation stay empty.

**A6. Field 6 (Authors): the canonical spelling is `Sarah Kovac`.** The ORCID registry record for `https://orcid.org/0000-0003-1714-5970` gives `family-name: Kovac` (no credit-name, no other-names), and Crossref (Patel et al. 2023 author list) and punchbowl's `pyproject.toml` independently agree. The repository's `CITATION.cff` still carries the upstream typo `Kovak`, and DataCite inherits it by deriving its creators from that file — worth reporting upstream. Author count remains exactly 9; her ORCID and her `NCAR High Altitude Observatory` / `https://ror.org/03773p874` affiliation are unchanged.

### B. Field 31/32 SPASE resolution log

Resolved against the live HSSI controlled vocabulary at `http://localhost/api/models/InstrumentObservatory/rows/all/` (7,648 rows fetched once to a file and filtered locally; every row's `identifier` is a `https://spase-metadata.org/...` URL). `type` 1 = instrument (Field 31), `type` 2 = observatory (Field 32).

| Candidate | Searched | Found | Ladder rung applied | Result |
|---|---|---|---|---|
| PUNCH (observatory) | `/\bPUNCH\b/`, `/Polarimeter to Unify/` | 2 same-resource observatory rows: `PUNCH Mission` (`.../NASA/Observatory/PUNCH`) and `Polarimeter to Unify the Corona and Heliosphere (PUNCH)` (`.../NASA/Observatory/PUNCH.html`) | **Rung 1** — disambiguated definitively by looking up HSSI's own existing row UUID `d882f614-6240-4f53-a423-295236b5c27b`, which *is* the `.html` row | Emitted `Polarimeter to Unify the Corona and Heliosphere (PUNCH)` + `https://spase-metadata.org/NASA/Observatory/PUNCH.html` (existing value preserved exactly; see Open Question O1) |
...
- **O4 — Funder.** No funding statement exists in the repository and DataCite `fundingReferences` is empty, so Field 25 is "Not found". If the user can confirm it, the likely value is `National Aeronautics and Space Administration` (ROR https://ror.org/027ka1x80), since PUNCH is a NASA Small Explorer mission and the code is copyright the PUNCH Science Operations Center. Not asserted, to avoid fabrication.
...
- **O8 — `Data Processing and Analysis: Wave Polarization Analysis`.** Included because the taxonomy's own indicator list for that value names "Stokes parameters" and polarization-ellipse quantities, all of which solpolpy computes. The category is more often used for plasma-wave polarimetry than for Thomson-scattering imaging polarimetry, so it is offered for validator judgment rather than presented as beyond question.
- **O9 — Field numbering in the refresh brief.** The brief referred to "Version (Field 27)" and "License (Field 12)". This file follows the authoritative `hssi-field-definitions` numbering, in which **Field 12 = Version**, **Field 15 = License**, and Field 27 = Related Publications. No content was affected.
