...
| Value | Evidence |
|---|---|
| Data Processing and Analysis | parent category required for hierarchy closure |
| Data Processing and Analysis:Data Access and Retrieval | core purpose: `SwarmRequest.get_between()`, `AeolusRequest.get_between()` fetch products from the VirES WPS endpoints (`_wps/wps_vires.py`) |
| Data Processing and Analysis:File Format Conversion | `ReturnedData.as_dataframe()/as_xarray()/as_xarray_dict()`, `to_file()`, `to_files()`, `to_netcdf()` convert served CSV/CDF/netCDF into pandas/xarray and back out to csv/cdf/nc (`_data_handling.py`) |
| Data Processing and Analysis:Processing | request chunking, response concatenation, `reshape_dataset()` reshaping of AUX_OBS/VOBS data into higher-dimensional objects, model-interpolation handling for 50 Hz MAGx_HR |
| Models and Simulations:Physics-Based | user-facing forward evaluation of spherical-harmonic geomagnetic field models, including composition/customisation and evaluation at arbitrary user coordinates (`SwarmRequest.eval_model`, `eval_model_for_cdf_file`, `available_models`, `get_model_info`; docs/geomagnetic_models.rst) |
| Mission-related:Distribution/Access | official ESA client for the VirES for Swarm / VirES for Aeolus services (README.rst; service run for ESA by EOX) |
| Mission-related | required parent of `Mission-related:Distribution/Access` |
| Models and Simulations | required parent of `Models and Simulations:Physics-Based` and `Models and Simulations:Empirical` |
| Models and Simulations:Empirical | `MAGNETIC_MODELS` exposes IGRF (an empirical reference field model), CHAOS-8 core/static/MMA/MIO, LCS-1, MF7, MCO/MLI/MIO/MMA_SHA_2C/2D/2E/2F and AMPS (`_client_swarm.py:1585-1616`, `MODEL_REFERENCES`) - all data-derived empirical spherical-harmonic models |
| Data Processing and Analysis:Analysis | documented analysis capabilities beyond retrieval: data-model residuals (`residuals=True`), spacecraft conjunction identification (`get_conjunctions`), orbit-number and orbit-time queries (`get_orbit_number`, `get_times_for_orbits`), collection metadata queries (`get_collection_info`, `available_times`) - docs/capabilities.rst |
| Data Processing and Analysis:Data Reduction | server-side subsetting/filtering (`set_range_filter`, `set_choice_filter`, `set_bitmask_filter`, `add_filter`, `AeolusRequest.set_bbox`) and resampling to a coarser cadence (`sampling_step` in `set_products`, `COLLECTION_SAMPLING_STEPS`) - docs/capabilities.rst "Data subsetting/filtering", "Data resampling" |
| Coordinate Transforms | required parent of the value below |
| Coordinate Transforms:Ionospheric | quasi-dipole magnetic coordinates and magnetic local time are delivered as user-requestable columns: `QDLat`, `QDLon`, `QDBasis`, `MLT`, `QDOrbitDirection`, `DipoleAxisVector`, `DipoleTiltAngle`, `NGPLatitude`, `NGPLongitude` (`AUXILIARY_VARIABLES`, `_client_swarm.py:1536-1575`); docs/available_parameters.rst states "``QDLat`` and ``QDLon`` are quasi-dipole coordinates" and "``MLT`` is calculated from the QDLon and the subsolar position". The transform is computed server-side and delivered by the client - the identical client/server relationship that justifies `Models and Simulations:Physics-Based`, so dropping one while keeping the other would be incoherent |
...
- The four finer-grained regions express the documented ionospheric, thermospheric, auroral, and Aeolus atmospheric product suites more precisely than Earth Atmosphere and Earth Magnetosphere alone.
...
- Evidence, Earth Auroral Subregion: the dedicated AEBS catalogue contains auroral-electrojet line profiles, peaks and boundaries, ground-magnetic-disturbance locations, and auroral-oval boundaries derived from field-aligned currents (`docs/available_parameters.rst:80-92`). These are retrievable collections, not merely contextual indices.
- Evidence, Earth Lower and Middle Atmosphere: the Aeolus client registers the full ALADIN L1A/L1B/L2A/L2B/L2C collection set and its ECMWF meteorological auxiliary collection (`src/viresclient/_client_aeolus.py:240-249`) and exposes Mie and Rayleigh wind and profile fields (`src/viresclient/_client_aeolus.py:31-35,61-66`). These Aeolus wind-profile and meteorological products support the finer atmospheric region; the repository itself does not give an explicit altitude range.
...
- Affiliation for Daniel Santillan is evidenced by `daniel.santillan@eox.at` (CITATION.cff) and the EOX IT Services GmbH copyright on the source headers and LICENSE. The ROR identifier matches the organisation recorded for Martin Pačes.
...
- Note: the repository/Zenodo title is `ESA-VirES/VirES-Python-Client` and SoMEF reports the repository name `VirES-Python-Client`. The package name `viresclient` is the correct HSSI software name and is retained.
...
- The prior HSSI text is preserved verbatim and unedited; the final sentence is appended because the earlier description named only Swarm and Aeolus and therefore materially understated scope. No maintainer-derived wording was rewritten, reordered or shortened.
...
- The selected value is not stale, incomplete or incorrect. The 2025-10-09 canonical file's alternative ("A Python package for easy access to Swarm & Aeolus products as xarray.Dataset") is maintainer-authored and appears verbatim in four primary sources (CITATION.cff `abstract`, the GitHub repository description, the Zenodo record description, and SoMEF's highest-confidence description), but the difference is stylistic rather than factual and does not justify replacing the selected description.
...
- This supersedes the stale prior HSSI value `v0.13.0` (2025-04-27, PID `https://doi.org/10.5281/zenodo.15292625`).
...
**Values retained from earlier evidence (14, stored lowercase):**
...
**Additional evidence-backed values (16):**
...
- Evidence for `WDC`: the AUX_OBS catalogue states that it provides ground magnetic observatory data from WDC and INTERMAGNET, including hourly values derived from both (`docs/available_parameters.rst:160-165`), and `_detect_AUX_OBS()` explicitly reports WDC access for those collections (`src/viresclient/_client_swarm.py:1919-1934`).
...
- All five are justified: `ReturnedDataFile`/`ReturnedData` support `("csv", "cdf", "nc")` via `to_file()`/`to_files()` (exercised over all `SUPPORTED_FILETYPES` in `tests/test_ReturnedData.py`), `to_netcdf()` writes netCDF, `eval_model` writes an HDF5 request payload with `h5py`, and `eval_model_for_cdf_file` writes an output CDF.
...
- The earlier HSSI value used `http://viresclient.readthedocs.io/`; the canonical HTTPS URL is recorded here.
...
- The ROR identifier `https://ror.org/03wd9za21` resolves to the European Space Agency.
...
- The earlier HSSI value `https://doi.org/10.3389/fspas.2022.1002697` is deliberately omitted here because Field 27 covers publications *different from* the reference publication. That DOI is recorded in Field 14, so retaining it here would duplicate it and misrepresent the field's meaning.
...
**Datasets:** Every entry is a DOI that the package itself cites for a collection or model it serves (`COLLECTION_REFERENCES` / `MODEL_REFERENCES` / `REFERENCES` in `src/viresclient/_client_swarm.py`).
...
| # | Observatory Name (verbatim) | Observatory Identifier | Why the software is designed to support it |
|---|---|---|---|
| 1 | Swarm : ESA mission | https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM | The mission the client is built for; the whole `SwarmRequest` API and the `SW_*` collection set |
| 2 | Aeolus | https://spase-metadata.org/SMWG/Observatory/AEOLUS | `AeolusRequest` and the Aeolus L1B/L2A/L2B/L2C collections (docs/available_parameters_aeolus.rst) |
| 3 | Swarm Alpha | https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM-A | Per-spacecraft collections `SW_OPER_MAGA_*`, `EFIA_*`, `TECA*`, `FACA*`, `AEJA*`, `MODA_SC`, `DNSA*`; `MISSION_SPACECRAFTS["Swarm"]` includes "A"; A-C conjunction pairs |
| 4 | Swarm Bravo | https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM-B | Per-spacecraft `...B...` collections; `CONJUNCTION_MISSION_SPACECRAFT_PAIRS = {(("Swarm","A"),("Swarm","B"))}` |
| 5 | Swarm Charlie | https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM-C | Per-spacecraft `...C...` collections, including the dual-satellite `SW_OPER_FAC_TMS_2F` (A-C) product |
| 6 | CHAMP | https://spase-metadata.org/SMWG/Observatory/CHAMP | `MAG_CHAMP` (`CH_ME_MAG_LR_3`), `DNS_ACC_CHAMP`, `WND_ACC_CHAMP`, `TEC_TIRO` (`CH_OPER_TEC_TMS_2F`), `VOBS_CH_1M/4M` |
| 7 | Gravity Recovery and Climate Experiment | https://spase-metadata.org/SMWG/Observatory/GRACE | `MAG_GRACE` (`GRACE_A_MAG`, `GRACE_B_MAG`), `DNS_ACC_GRACE`, `WND_ACC_GRACE`, `TEC_TIRO`, `NE_TIRO`; `MISSION_SPACECRAFTS["GRACE"] = ["1","2"]` for orbit-number queries |
| 8 | Gravity Field and Steady-State Ocean Circulation Explorer | https://spase-metadata.org/SMWG/Observatory/GOCE | `MAG_GOCE` (`GO_MAG_ACAL_CORR`), `MAG_GOCE_ML`; `MISSION_SPACECRAFTS["GOCE"]` supported by `get_orbit_number` |
| 9 | International Real-time Magnetic Observatory Network | https://spase-metadata.org/SMWG/Observatory/Ground/INTERMAGNET | `AUX_OBSH`, `AUX_OBSM`, `AUX_OBSS` collections keyed by IAGA observatory code (`IAGA_CODES` in `_data/config_swarm.json`), plus `available_observatories()`; README.rst - "INTERMAGNET ground magnetometers via the ``AUX_OBS`` collection" |

**Resolved SPASE decisions:**
- **`.html`-vs-bare duplicate identifiers: both entries are bound to the bare, registered SPASE identifier.** The two rows recorded above are `https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM` and `https://spase-metadata.org/SMWG/Observatory/AEOLUS`. Each of these resources was, for a period, registered twice in the HSSI vocabulary - once under its bare SPASE identifier and once under a `.html` variant (`.../CDPP-AMDA/SWARM.html`, `.../SMWG/Observatory/AEOLUS.html`) - and this record's entries were bound to the `.html` variants. The `.html` form is not a SPASE identifier: it is the landing-page URL that serves the SPASE resource, captured as an artifact of the submission path. The registry mints only the bare form, which is what the maintained upstream records carry (`spase://CNES/Observatory/CDPP-AMDA/SWARM`, `spase://SMWG/Observatory/AEOLUS`). This record's earlier judgement - decline to repoint the bindings, because a duplication affecting the shared vocabulary is not something to fix from one software record - was correct, and it was superseded the right way: the duplicates were reconciled vocabulary-wide onto the registry's identifiers, and these two entries follow that consolidation. The bare identifier is final here; the `.html` form is not to be reintroduced.
- **Swarm display name "Swarm : ESA mission": the cost was foreseen and is accepted.** This record previously warned that moving Swarm onto the bare row would trade the clean display name "Swarm" for "Swarm : ESA mission". That warning was sound, and the awkward label is exactly what this record now shows. It stands anyway, because the identifier is the durable half of the association and the name is not: "Swarm : ESA mission" is the `AlternateName` carried by the upstream SPASE record itself (whose `ResourceName` is the plain "Swarm"), so the string originates upstream rather than here. A local rename would be transient - a refresh of the vocabulary from upstream reinstates the upstream form - whereas an unregistered identifier would be a lasting defect. Correct identity outweighs a tidier label; renaming this row is not to be re-proposed from this record.
- **Prior assumption corrected:** the two live observatory entries were never identifier-less rows — they carried the `.html` landing-page variants named above, since consolidated onto the bare registered identifiers.
...
- **GRACE / GOCE bare-vs-`.html`.** Both once existed in bare and `.html` form; the bare rows (and their names) are the recorded values, and the `.html` duplicates were retired in the vocabulary-wide consolidation.
