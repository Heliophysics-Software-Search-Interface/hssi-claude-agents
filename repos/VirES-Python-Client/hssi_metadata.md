...
|---|---|---|---|---|
| 1 | Swarm : ESA mission | https://spase-metadata.org/CNES/Observatory/CDPP-AMDA/SWARM | `LIVE` | The mission the client is built for; the whole `SwarmRequest` API and the `SW_*` collection set |
| 2 | Aeolus | https://spase-metadata.org/SMWG/Observatory/AEOLUS | `LIVE` | `AeolusRequest` and the Aeolus L1B/L2A/L2B/L2C collections (docs/available_parameters_aeolus.rst) |
...
11. **Field 32 - `.html`-vs-bare duplicate rows: the Swarm and Aeolus entries are bound to the bare, registered SPASE identifiers.** Leaving the `.html` bindings alone was the right call for a single record to make, and the duplication was afterwards resolved across the vocabulary, so the entries follow the registry's identifiers. The predicted cost - Swarm displaying as "Swarm : ESA mission" rather than "Swarm" - is accepted, because that name comes from the upstream SPASE record's `AlternateName` and a local rename would not survive a vocabulary refresh.
