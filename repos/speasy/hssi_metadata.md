...
**Applied to HSSI:** 2026-07-24 — `PATCH http://localhost/api/data/software/6adc2c82-61b3-41b4-89ca-62862bfe9b9f/` returned HTTP 200 with 16 fields updated (`authors`, `data_sources`, `development_status`, `input_formats`, `interoperable_software`, `keywords`, `license`, `operating_system`, `output_formats`, `related_instruments`, `related_observatories`, `related_phenomena`, `related_publications`, `related_software`, `software_functionality`, `version`). Roundtrip-verified: all expected counts matched, no pre-existing value dropped from any multi-valued field, and the 16 deliberately-omitted fields (including Description and Concise Description) are byte-for-byte unchanged.

**Seeds used (this is a seeded refresh, not a blank-slate extraction):**
1. **Live HSSI record** — `GET http://localhost/api/view/software/6adc2c82-61b3-41b4-89ca-62862bfe9b9f/` (local DB mirrors the 2026-07-23 production backup). Authoritative baseline for scalar/editorial fields.
...
**Change legend used throughout:**
- `UNCHANGED` — value matches live HSSI exactly; no patch needed.
- `ADDITION` — new value not present in live HSSI (includes values carried forward from the prior canonical file).
- `PROPOSED REPLACEMENT` — live HSSI value is stale/incorrect; replacement is evidence-backed, with the reason stated.
- `CANDIDATE (user decision)` — plausible but not firmly evidenced; was not auto-proposed as a patch. **All such markers were dispositioned on 2026-07-24 — see "Decisions applied" near the end of this file for the authoritative outcome of each.**
- `Note:` — considered-and-dropped values, with the reason, for audit trail.

**Release-state caveat used throughout:** the repository's `main` branch at the extraction revision contains capabilities that are **not in the v1.7.1 release** (2025-12-18). Values whose sole or primary evidence is post-release code are tagged `[main-branch, unreleased as of v1.7.1]`. Verified post-release-only: `speasy/core/hapi/` HAPI server client, HAPI **binary** codec, ISTP **netCDF** codec, and the `cdpp3dview` data provider. Verified already released in v1.7.1: HAPI **CSV** codec, ISTP CDF codec, CSA-over-TAP, bundled `cda.yaml` archive definitions, UiowaEphTool, SSCWeb coordinate systems, signal filtering/resampling, line + colormap plotting, WASM/Pyodide support.
...
**Status:** N/A for an update — HSSI does not expose the original submitter through the view API. The Speasy wording in HSSI (name, description, concise description) is treated as intentional submitted editorial content and is preserved verbatim.
...
**Values — 16 total (10 unchanged + 6 additions):**

*Formatting note:* HSSI stores/displays these with a space after the colon (e.g. `Data Processing and Analysis: Analysis`). Below they are written in the canonical form from the HSSI field definitions (no space). This is a display/storage normalization, **not** a value change — the 10 unchanged values must not be diffed on the colon-space.

**UNCHANGED (from existing HSSI record) — 10:**
...
**Values — 3 (all unchanged):**
1. `Earth Magnetosphere` — UNCHANGED
2. `Interplanetary Space` — UNCHANGED
3. `Planetary Magnetospheres` — UNCHANGED
...
**CANDIDATE (user decision) — `Solar Environment`:** Not proposed as a patch. Speasy's dynamic inventory reaches solar-mission datasets through CDAWeb/AMDA, `docs/examples/*.ipynb` and `tests/test_cdpp3dview.py` reference SOHO/SDO/STEREO, and the developers' own EPSC-DPS 2025 abstract frames the tool as serving "planetary and heliospheric" data. However the repository contains **no solar-atmosphere-specific support**, and the inner-heliosphere in-situ data (PSP/SOLO/STEREO SEP and magnetic field) is already covered by Interplanetary Space. Flagged rather than added, per "prefer evidence over speculation".
...
**Values — 6 authors (2 unchanged + 4 additions).** This is the single largest correction in this refresh: live HSSI has 2 authors, while the repository's current `CITATION.cff`, the Zenodo v1.7.1 record, and the DataCite concept-DOI record all list **6**. Matched by ORCID first, then normalized name; affiliations unioned per matched author. No author present in any source has been dropped.

**Author 1 — Alexis Jeandet** — UNCHANGED (author + affiliation)
- **Name:** Alexis Jeandet
...
- **Source:** From existing HSSI record (organization row `9e5aa6d0-8571-4948-942a-9abd57d3a45c`). Re-verified: `CITATION.cff`, `pyproject.toml` `authors`/`maintainers`, `AUTHORS.rst` (Development Lead), Zenodo + DataCite creators, and the author's ORCID record (current employment "Laboratoire de Physique des Plasmas", ROR `05c95bg36`, since 2008 — an exact ROR match to the existing HSSI affiliation).

**Author 2 — Alexandre Schulz** — author UNCHANGED; **affiliation is an ADDITION**
...
- **Source:** Author from existing HSSI record (affiliation list is currently **empty** there). Affiliation carried forward from the prior `hssi_metadata.md` and supported by `AUTHORS.rst` (`alexandre.schulz@irap.omp.eu`). The full expanded name and ROR match an **existing HSSI organization row** (`bfbd0415-674b-4b3d-a7cf-cf48b7d28576`), so no new organization is created.
- *Note:* the author's ORCID lists a different current employment (Université Sorbonne Paris Nord, ROR `0199hds37`, since 2022). The repo-stated IRAP affiliation is the one relevant to his Speasy contribution (the AMDA web service). Flagged for the user in case both should be recorded.

**Author 3 — Nicolas Aunai** — ADDITION
- **Name:** Nicolas Aunai
...
- **Affiliation NOT sent in the 2026-07-24 PATCH (user-approved decision).** His HSSI Person row already carried an affiliation to a *different* organization row naming the same lab — "CNRS / Laboratory of Plasma Physics" (empty identifier). Because the update endpoint only ever `.add()`s affiliations and can never remove one, sending the LPP row above would have left him permanently showing two affiliations for the same institution. The `affiliation` key was therefore omitted for this author; his pre-existing affiliation is retained untouched (roundtrip-verified: exactly 1 affiliation). Resolving the duplicate organization rows would require the CSV/manual database workflow.

**Author 4 — Benjamin Renard** — ADDITION
- **Name:** Benjamin Renard
- **Author Identifier:** Not found (no ORCID in `CITATION.cff`, Zenodo, or DataCite)
- **Affiliation:**
  - **Organization:** Institut de Recherche en Astrophysique et Planétologie
  - **Affiliation Identifier:** `https://ror.org/05hm2ja81`
- **Source:** `CITATION.cff` (4th author), Zenodo v1.7.1 creators, DataCite creators, `AUTHORS.rst` (Contributors -> AMDA Webservice, `benjamin.renard@irap.omp.eu`). Also a Speasy committer (`@brenard-irap` in `HISTORY.rst` for 1.5.0/1.5.2) and co-author of the EPSC-DPS 2025 Speasy abstract. Affiliation matches the existing HSSI IRAP organization row.

**Author 5 — Vincent Génot** — ADDITION
- **Name:** Vincent Génot
...
- *Could not verify affiliation:* the ORCID record's `employments` group is empty and the repository states no affiliation for him. He is the lead author of the AMDA reference paper (`10.1016/j.pss.2021.105214`), but Crossref returns no affiliation strings for it. **Deliberately left "Not found" rather than assumed to be IRAP.** (`CANDIDATE (user decision)`: Institut de Recherche en Astrophysique et Planétologie / `https://ror.org/05hm2ja81`.)

**Author 6 — Nicolas André** — ADDITION
- **Name:** Nicolas André
...
- *Could not verify affiliation:* the ORCID record lists current employments "Institut Supérieur de l'Aéronautique et de l'Espace" (ROR `https://ror.org/04gyj6s21`, since 2024-03) and "Centre National de la Recherche Scientifique" (since 2009), neither of which is stated in the repository or obviously the affiliation under which he contributed to Speasy. **Deliberately left "Not found" rather than guessed.** (`CANDIDATE (user decision)`: either of the two ORCID-listed employments, or Institut de Recherche en Astrophysique et Planétologie.)
...
- **Status:** UNCHANGED — submitted editorial wording preserved verbatim.
...
**Correction to an earlier source note (validator finding W1):** the grammar artifacts "nowaday" and "one of the major bottleneck" are **not** inherited from the current upstream text. They are legacy phrasing specific to the previously submitted HSSI description. The upstream README was grammatically corrected in commit `6caade95bbc325a99e29ccca04d8a2740d6add8c` ("Pick readme improvments from @nicolasaunai", 2023-10-30) and now reads "...considering the diverse array of missions and instruments available nowadays, it proves to be one of the major bottlenecks..." (`README.md:18-24`, `docs/index.rst:66-69`). The PyHC registry description reproduces only the first sentence and never contained the "bottleneck" clause at all.

**CANDIDATE (user decision):** whether to replace the submitted description with the current, grammatically corrected upstream README wording. The submitted value is factually accurate, so this is an editorial-currency judgment, not a correctness fix.
...
**Source:** From existing HSSI record (organization row `ee990b81-8115-4ab4-8a7f-68a4d0bb345d`). Correct per the HSSI field guidance (DOI obtained via the GitHub-Zenodo workflow); DataCite `attributes.publisher = "Zenodo"`.

---

### 12. Version (RECOMMENDED) — **highest-priority change in this refresh**

Live HSSI holds `v1.6.1` (released 2025-09-05). Two releases have shipped since. All four subfields are **PROPOSED REPLACEMENT**.
...
- **Version Number:** `v1.7.1`
  **PROPOSED REPLACEMENT** — reason: v1.6.1 is two releases stale. Evidence: `VERSION` file = `1.7.1`; `pyproject.toml` `version = "1.7.1"`; `speasy/__init__.py` `__version__ = '1.7.1'`; `CITATION.cff` `version: 1.7.1`; newest git tag `v1.7.1`; DataCite concept DOI `attributes.version = "v1.7.1"`; Zenodo latest version `v1.7.1`.
...
- **Version Date:** `2025-12-18`
  **PROPOSED REPLACEMENT.** Evidence: GitHub Releases API `v1.7.1 published_at = 2025-12-18T14:05:16Z`; Zenodo record 17978034 `metadata.publication_date = "2025-12-18"` and `created = 2025-12-18T14:05:35Z`; DataCite `dates[] Issued = 2025-12-18`; `CITATION.cff` `date-released: 2025-12-18`; git tag `v1.7.1` dated 2025-12-18.
...
- **Version PID:** `https://doi.org/10.5281/zenodo.17978034`
  **PROPOSED REPLACEMENT** — reason: the live value is the v1.6.1 version DOI. Resolved from the concept DOI as instructed: `GET https://zenodo.org/api/records/4118780` resolves to the latest version, record **17978034**, with `"doi": "10.5281/zenodo.17978034"`, `"conceptdoi": "10.5281/zenodo.4118780"`, `"metadata.version": "v1.7.1"`. Independently confirmed by `CITATION.cff` `doi: 10.5281/zenodo.17978034`. **The v1.6.1 DOI `10.5281/zenodo.17065425` is NOT reused.**

- **Version Description:** **PROPOSED REPLACEMENT** — formatted to match the style of the existing HSSI entry (version header, then `- <change> by @<author> in #<PR>`), covering both releases published since HSSI was last updated:
...
- **Value:** Not found
- **Status:** UNCHANGED (live HSSI: `referencePublication` is `null`)
...
- **Status:** ADDITION (live HSSI: `license` is `null`)
...
**UNCHANGED (from existing HSSI record) — 18, reproduced verbatim in HSSI's stored casing so they do not read as diffs:**
...
**ADDITION — 11 (set-union of PyHC curated keywords + fresh repo evidence):**
...
*Casing note:* HSSI title-cases keywords for display. New values are written lowercase in their natural form; existing values are reproduced exactly as stored.

---

### 17. Data Sources (OPTIONAL)

**UNCHANGED (from existing HSSI record) — 6:**
...
**UNCHANGED (from existing HSSI record) — 4:** `ascii`, `CDF`, `csv`, `Other`
...
**UNCHANGED (from existing HSSI record) — 3:** `ascii`, `CDF`, `csv`
...
**UNCHANGED (from existing HSSI record) — 4:** `Linux`, `Mac`, `Operating System Independent`, `Windows`
...
**Source for the unchanged four:** From existing HSSI record; re-verified against `.github/workflows/tests.yml`, whose matrix is `os: [macos-latest, windows-latest, ubuntu-latest]` x `python-version: [3.10, 3.11, 3.12, 3.13, 3.14]`.
...
- **Value:** `Solar Wind`
- **Status:** ADDITION (live HSSI: `relatedPhenomena` is empty)
- Exact live controlled-vocabulary row, verified 2026-07-24: `Solar Wind` / `73b7fe8c-ee03-46eb-83d8-0ce77956245a`.

**Reasoning (re-checked against the LIVE vocabulary — validator finding W3):** the earlier reasoning enumerated a stale list from the static form-fields reference. The live controlled list (`GET http://localhost/api/models/Phenomena/rows/all/`, 7 rows) is: **Coronal Heating, Coronal Mass Ejections, Geomagnetic Storms, Solar Corona, Solar Flares, Solar Wind, X-ray emission** — there is no "Coronal Holes", and it adds "Geomagnetic Storms" and "Solar Wind", which the stale check never considered.
...
- **Value:** `Active`
- **Status:** ADDITION (live HSSI: `developmentStatus` is `null`)
...
**Source:** From existing HSSI record (organization row `cf31bdf7-1fa2-4f13-9a2b-a8d5e94384c1`). Re-verified against the README "Credits" section: "The development of Speasy is supported by the CDPP (http://www.cdpp.eu/)."
...
- **Status:** UNCHANGED (live HSSI: `award` is empty)
...
**ADDITION — 1 submitted** (live HSSI before this refresh: `relatedPublications` was empty). Entry 1 below was patched on 2026-07-24; entry 2 was **decided against and NOT submitted** (see its note).
...
2. `https://doi.org/10.1051/0004-6361/202141095`
   ...
   **DECIDED — OMITTED, not submitted** (2026-07-24, user-approved): this paper does not itself describe or cite Speasy — it is the science result that a documented Speasy example reproduces. Field 27 was read strictly as "publications that describe, cite, or use the software". Retained here as documented evidence only; it is deliberately absent from HSSI.
...
- **Value:** Not found
- **Status:** UNCHANGED (live HSSI: `relatedDatasets` is empty)
...
**ADDITION — 5** (live HSSI: `relatedSoftware` is empty). Every entry below passed the Field 29/30 relevance gate; the gate reasoning is given per entry, and every rejection is recorded in the Note for audit purposes.
...
**ADDITION — 3** (live HSSI: `interoperableSoftware` is empty). This field is gated on a *demonstrated exchange*, not on dependency presence; the specific evidence is named for each entry.
...
**ADDITION — 19** (live HSSI: `relatedInstruments` is empty). Every entry is resolved to exactly one SPASE row and recorded with that row's canonical `name` copied verbatim plus its SPASE `identifier` (the reliable de-duplication key on submission). Vocabulary source: `GET http://localhost/api/models/InstrumentObservatory/rows/all/` (7,648 rows; `.html` duplicates normalised to the bare identifier; filtered to `type = 1`).
...
**`CANDIDATE (user decision)` — 2 instruments genuinely supported but ABSENT from the SPASE vocabulary (therefore NOT submittable as clean values):**
...
- **Arase/ERG PWE/HFA** (Plasma Wave Experiment - High Frequency Analyzer) — `cda.yaml` ships `erg_pwe_hfa_l3_1min`; exercised by `tests/test_direct_archive_downloader.py`, `tests/test_direct_archive_inventory.py`, `tests/test_codecs.py`, and `tree.archive.cda.Arase_ERG.PWE.HFA.erg_pwe_hfa_l3_1min.ne_mgf`.
  ...
  The vocabulary contains the **observatory** row `SMWG/Observatory/ARASE` (added under Field 32) but **zero Arase/ERG instrument rows** — verified by both identifier-path and name search across all 7,648 rows. Submitting these as bare names would create new non-SPASE instrument rows in HSSI's controlled vocabulary, so they are flagged for an explicit user decision rather than proposed.
...
**16 values — 9 unchanged + 7 additions.** Each resolves to exactly one SPASE row (`type = 2`), with the canonical `name` copied verbatim.

**UNCHANGED (from existing HSSI record) — 9.** Each was mapped from its HSSI row UUID back through the vocabulary to confirm the exact SPASE row currently bound, then re-verified as still supported by the repository at the extraction revision. **No existing value is proposed for removal.**
...
- **Consistency check:** THEMIS and Arase are added while STEREO/BepiColombo/Equator-S are not, because the former have shipped package data and/or broad multi-file test coverage while the latter appear exactly once as fixtures for generic features. Juno is included on the strength of two independent sources (a feature test plus a developer statement) and is explicitly flagged as the weakest addition.
...
**Source:** From existing HSSI record — submitted value preserved. **Verified live: HTTP 200.** Identical to the PyHC registry `logo` field. The repository also contains `logo/logo_speasy.svg` (also HTTP 200, used in the README and docs headers), but Field 33 takes a single URL and the submitted PNG is kept.
...
### Field-by-field change status (all 33 fields present)

| # | Field | Status |
|---|---|---|
| 1 | Submitter | N/A (placeholder; not exposed by the HSSI view API) |
| 2 | Persistent Identifier | UNCHANGED |
| 3 | Code Repository | UNCHANGED |
| 4 | Software Functionality | 10 UNCHANGED + **6 ADDITIONS** (incl. a required missing parent) |
| 5 | Related Region | 3 UNCHANGED (+1 candidate flagged, not proposed) |
| 6 | Authors | 2 UNCHANGED + **4 ADDITIONS**; 1 affiliation ADDITION |
| 7 | Software Name | UNCHANGED |
| 8 | Description | UNCHANGED (submitted editorial wording preserved) |
| 9 | Concise Description | UNCHANGED (submitted editorial wording preserved) |
| 10 | Publication Date | UNCHANGED |
| 11 | Publisher | UNCHANGED |
| 12 | Version | **PROPOSED REPLACEMENT (all 4 subfields)** |
| 13 | Programming Language | UNCHANGED |
| 14 | Reference Publication | Not found (unchanged) |
| 15 | License | **ADDITION** (was null) |
| 16 | Keywords | 18 UNCHANGED + **11 ADDITIONS** |
| 17 | Data Sources | 6 UNCHANGED + **3 ADDITIONS** |
| 18 | Input File Formats | 4 UNCHANGED + **2 ADDITIONS** |
| 19 | Output File Formats | 3 UNCHANGED + **3 ADDITIONS** |
| 20 | Operating System | 4 UNCHANGED + **1 ADDITION** |
| 21 | CPU Architecture | UNCHANGED |
| 22 | Related Phenomena | **ADDITION (1)** — `Solar Wind` |
| 23 | Development Status | **ADDITION** (was null) |
| 24 | Documentation | UNCHANGED (verified HTTP 200) |
| 25 | Funder | UNCHANGED (no ROR exists for CDPP) |
| 26 | Award Title | Not found (unchanged) |
| 27 | Related Publications | **ADDITION (1)** — EPSC-DPS 2025 abstract only; Louarn et al. 2021 omitted by decision |
| 28 | Related Datasets | Not found (unchanged) |
| 29 | Related Software | **ADDITION (5)** |
| 30 | Interoperable Software | **ADDITION (3)** |
| 31 | Related Instruments | **ADDITION (19)** + 2 flagged candidates + 1 documented drop |
| 32 | Related Observatories | 9 UNCHANGED + **7 ADDITIONS** |
| 33 | Logo | UNCHANGED (verified HTTP 200) |

**No existing HSSI value is proposed for removal anywhere in this refresh.** The only seed value not carried forward is the prior file's ACE/SWEPAM instrument (Field 31), which is absent from live HSSI and no longer supported by the repository; that drop is documented in place.

### Decisions applied — all resolved 2026-07-24 (user-approved)

Every `CANDIDATE (user decision)` marker left in the field bodies above was dispositioned before the PATCH. The markers are retained as evidence trails; the outcomes below are authoritative and match the live HSSI record.

| # | Item | Decision |
|---|---|---|
| 1 | **Field 31 — Arase/ERG LEPE and PWE/HFA:** genuinely supported (shipped `cda.yaml` definitions) but **absent from the SPASE vocabulary** | **OMITTED** — would have created non-SPASE instrument rows |
| 2 | **Field 5 — `Solar Environment`** | **OMITTED** — plausible but not firmly evidenced |
| 3 | **Field 27 — Louarn et al. 2021** | **OMITTED** — does not describe or cite Speasy |
| 4 | **Field 14 vs 27 — EPSC-DPS 2025 abstract** | **KEPT in Field 27**; not promoted to Reference Publication. Field 14 remains empty |
| 5 | **Field 6 — affiliations for Vincent Génot and Nicolas André** | **OMITTED** — both authors submitted with ORCIDs but no affiliation, rather than guessing |
| 6 | **Values tagged `[main-branch, unreleased as of v1.7.1]`** (Fields 16, 17, 18, 19, part of Field 4): netCDF3/4 input+output, HAPI data source, HAPI-binary output, `hapi`/`netcdf` keywords | **KEPT and submitted** — the record's Code Repository points at `main` |
| 7 | **Field 32 — `Juno Orbiter` and `Voyager 1 & 2` granularity** | **KEPT as written** — both SPASE-resolved; Voyager as one mission-level row |
| 8 | **Field 22 — `Solar Wind`** (surfaced by validator finding W3) | **ADDED** |
| 9 | **Field 8 — description wording** | **KEPT** the submitted HSSI wording verbatim; not refreshed to the corrected upstream README text |
| 10 | **Field 6 — Nicolas Aunai's affiliation** | **NOT SENT** — avoids a permanent duplicate affiliation for the same lab (see Author 3) |

### Could not verify
- **Affiliations for Vincent Génot** (ORCID `employments` group is empty; no repository statement) **and Nicolas André** (ORCID lists ISAE-SUPAERO and CNRS, neither stated in the repo nor obviously his Speasy affiliation). Crossref returns no affiliation strings for the AMDA reference paper.
- **An author identifier for Benjamin Renard** — no ORCID in `CITATION.cff`, Zenodo, or DataCite.
